...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateAll() {
    ...
    if (pie) {
      var labels = selCats.filter(function (c) {
        return pcm.hasOwnProperty(c);
      });
      pie.data.labels = labels;
      pie.data.datasets[0].data = labels.map(function (c) {
        return pcm[c];
      });
      pie.update();
    }
    ...
    if (topChart) {
      ...
      topChart.data.labels = top.map(function (x) {
        return x.merchant;
      });
      topChart.data.datasets[0].data = top.map(function (x) {
        return x.total;
      });
      topChart.update();
    }
    ...
    if (dailyChart && rawDaily.length) {
      dailyChart.data.labels = rawDaily.map(function (d) {
        return d.date;
      });
